...
\ref{sec:lagrangian-dynamics}, in principle, we could modulate $f_k$ to move
the ZMP in an area which is defined by the following set:
\begin{equation}
    \mathcal{S}_Z = \left\{ \bm{p}_Z \mid \bm{p}_Z = \bm{p}_C + \sum_{k=1}^K \gamma_k (\bm{p}_k - \bm{p}_C), \; \gamma_k > 0  \right\}.
\end{equation}

The set $\mathcal{S}_Z$, which is a polyhedral cone with apex $\bm{p}_C$,
