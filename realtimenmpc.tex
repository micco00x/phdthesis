...
Let us simplify the notation of the NLP \eqref{eq:transcribed-OCP}
by expressing it as
\begin{equation}
    \label{eq:NLP}
    \begin{aligned}
...
Algorithm \ref{alg:SQP} summarizes the SQP method. Note that the most 
...
\begin{algorithm}
	...
	\label{alg:SQP}
	\KwIn{estimated state of the system $\hat{\bm{x}}_k$ and an initial guess $\tilde{\bm{x}}_k$}
    $\kappa \leftarrow 0$\;
    $\bm{x}_\kappa \leftarrow \tilde{\bm{x}}_k$\;
    \While{\textit{convergence criterion not satisfied}}{
        compute $\bm{H}_{\kappa}, \bm{g}(\bm{x}_{\kappa}, \hat{\bm{x}}_k), \bm{h}(\bm{x}_{\kappa})$ and sensitivities $\nabla \bm{f}(\bm{x}_{\kappa}), \nabla \bm{g}({\bm{x}_{\kappa}}), \nabla \bm{h}({\bm{x}_{\kappa}})$\;
        $\Delta \bm{x}_{\kappa} \leftarrow $ construct and solve the QP \eqref{eq:QP}\;
        $\bm{x}_{\kappa+1} \leftarrow \bm{x}_{\kappa} + \alpha_k \Delta \bm{x}_{\kappa}$\;
...
The main idea behind RTI is to exploit the fact that NMPC solves OCPs which 
...
\begin{algorithm}
	...
	\KwIn{previous solution $\bm{x}_{k-1}$}
